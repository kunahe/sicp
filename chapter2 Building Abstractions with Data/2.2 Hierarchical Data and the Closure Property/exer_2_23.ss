...
procedure to the elements are not used at all¡ªfor-each is used with procedures that perform an 
...
(define (for_each procedure list)
    (if (not (null? list))        
        (begin (procedure (car list))
            (for_each procedure (cdr list))
        )
    )
)
